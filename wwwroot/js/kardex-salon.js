...
function inicializarBusqueda() {
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        const rows = document.querySelectorAll('#kardexSalonTable tbody tr');
...
function calcularFilaLocal(row) {
    ...
    const invInicial = parseInt(row.children[4].textContent) || 0;
...
function actualizarProgreso() {
    const rows = document.querySelectorAll('#kardexSalonTable tbody tr:not([style*="display: none"])');
...
function mostrarModalFaltantes() {
    ...
    if (listaElement) {
        ...
        utensiliosConFaltantes.forEach(item => {
            ...
            itemDiv.className = 'faltante-item';
            itemDiv.innerHTML = `
                <div class="faltante-info">
                    ...
                    <div class="faltante-detalles">
                        <div class="faltante-nombre">${item.descripcion}</div>
                        <div class="faltante-codigo">${item.codigo}</div>
                    </div>
                </div>
                <div class="faltante-cantidad">
    ...
    // Limpiar textarea
    const textareaElement = document.getElementById('descripcionFaltantesTexto');
    if (textareaElement) {
...
function continuarAPersonalPresente() {
    ...
    if (utensiliosConFaltantes.length > 0 && !descripcionFaltantes.trim()) {
        showNotification('Por favor, describa qué pasó con los utensilios faltantes', 'warning');
        return;
    }
    
    // Guardar descripción en sessionStorage para pasarla a Personal Presente
